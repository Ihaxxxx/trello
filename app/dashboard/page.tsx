...
function DashboardPage() {
  ...
  const leftTeam = searchParams.get("leftTeam");
  ...
  const [columns, setColumns] = useState<ColumnWithTask[] | null>(null);
  ...
  const boardsWithTaskCount = boards.map((board) => {
    const count =
      taskCount.find((tc: any) => tc.boardId === board.id)?.taskCount || 0;
    return { ...board, taskCount: count };
  });

  const filteredBoards = boardsWithTaskCount.filter(
    (board: BoardWithOptionalTaskCount) => {
  ...
  function isNewBoard(createdAt: string) {
    const createdTime = new Date(createdAt).getTime();
    const now = Date.now();
    const diffHours = (now - createdTime) / (1000 * 60 * 60); // ms → hours
    return diffHours <= 24;
  }
        ...
        <div className="mb-6 sm:mb-8">
          ...
          {boards.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 sm:gap-6">
              ...
              <Card onClick={handleCreateBoard} className="border-2 border-dashed border-gray-300 hover:border-blue-400 transition-colors cursor-pointer">
            ...
            <div>
              ...
              <Card onClick={handleCreateBoard} className="mt-4 border-2 border-dashed border-gray-300 hover:border-blue-400 transition-colors cursor-pointer">
